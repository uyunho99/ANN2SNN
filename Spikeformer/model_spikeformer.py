...
class SpikeSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_output_weights = F.softmax(
            q @ k.transpose(-2, -1) / math.sqrt(self.dim), dim=-1)
...
class SpikingTransformerEncoderLayer(nn.TransformerEncoderLayer):
    def __init__(self, dim, num_head, dim_feedforward=2048, dropout=0.1):
        super().__init__(dim, num_head, dim_feedforward, dropout)
        self.self_attn = SpikeSelfAttention(dim, num_head, dropout)
